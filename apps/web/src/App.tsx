  ...
  return <>
    ...
    <div className="customer-booking-layout">
      <section className="booking-form-panel customer-booking-main">
        ...
        <section className="customer-section customer-calendar-card"><div className="customer-section-heading"><div><span className="customer-eyebrow">ขั้นตอนที่ 2</span><h2>เลือกวันและเวลาที่สะดวก</h2><p>เราจะแสดงเฉพาะช่วงเวลาที่มีรถพร้อมให้บริการ</p></div><span className="required-note">* จำเป็น</span></div><div className="booking-month customer-calendar-toolbar"><button className="calendar-arrow" aria-label="เดือนก่อนหน้า"><ArrowLeft size={17} /></button><div><b>กันยายน 2569</b><small>เลือกวันที่มีป้าย “ว่าง”</small></div><button className="calendar-arrow" aria-label="เดือนถัดไป"><ArrowRight size={17} /></button></div><div className="availability-legend"><span><i className="legend-dot open" /> ว่าง</span><span><i className="legend-dot limited" /> เหลือคิวน้อย</span><span><i className="legend-dot closed" /> เต็ม</span></div><div className="availability-days customer-date-grid">{days.map((day) => <button key={day.date} disabled={day.tone === 'closed'} className={selectedDate === day.date ? `availability-day customer-date-card selected ${day.tone}` : `availability-day customer-date-card ${day.tone}`} onClick={() => { setSelectedDate(day.date); setAvailability(null); setSelectedSlot(day.tone === 'limited' ? '13:00–15:30' : '09:00–11:30'); }}><span>{day.label.split(' ')[0]}</span><strong>{day.label.split(' ')[1]}</strong><b>{day.state}</b></button>)}</div><div className="slot-area customer-slot-area"><div className="slot-heading"><div><span>ช่วงเวลาที่ว่าง · {formatThaiDate(selectedDate)}</span><small>{availability ? `รถพร้อมให้เลือก ${availability.capacity} คัน` : 'เลือกช่วงเวลาที่เหมาะกับคุณ'}</small></div><Clock3 size={21} /></div><div className="customer-time-grid">{slots.length ? slots.map((slot) => <button key={slot.label} disabled={!slot.available} className={selectedSlot === slot.label ? 'slot customer-time-slot selected' : 'slot customer-time-slot'} onClick={() => setSelectedSlot(slot.label)}><span><Clock3 size={17} /><b>{slot.label}</b></span><small>{slot.available ? `เหลือ ${slot.remaining} คัน` : 'เต็มแล้ว'}</small></button>) : <div className="empty-slot">วันนี้เต็มแล้ว ลองเลือกวันที่ใกล้เคียง</div>}</div></div></section>
